...
### Javascript Beautifier for Node.js

Forked on GitHub and modified by Nathan Cartwright, <fshost@yahoo.com>.
Ported to use node by Carlo Zottmann, <carlo@municode.de>.
Original Rhino version written by Patrick Hof, <patrickhof@web.de>. 
adapted from JS Beautifier, written by Einar Lielmanis, <einar@jsbeautifier.org>, http://jsbeautifier.org/
Originally converted to javascript by Vital, <vital76@gmail.com>

Usage:
	require('nodeBeautify').beautify(source, options);
...
exports.beautify = function(source, options) {
	for (var key in options)
		settings[key] = options[key];
	settings.indentChar = (settings.indentSize === 1) ? '\t' : ' ';
    source = source.replace(/^\s+/, '');
	return js_beautify(source, settings);
}
...
function js_beautify(js_source_text, options) {
  var input,
    ...
    n_newlines,
    opt_braces_on_own_line = options.bracesOnOwnLine,
    opt_indent_size = options.indentSize,
    opt_indent_char = options.indentChar,
    opt_preserve_newlines = options.preserveNewlines,
    opt_indent_level = options.indentLevel,
    opt_space_after_anon_function = options.spaceAfterAnonFunction,
    opt_keep_array_indentation = options.keepArrayIndentation,
